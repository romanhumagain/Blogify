...
def send_otp_for_registered_user(request):
    ...
    if email is not None:
        ...
        if email_sent:
            return Response(
                {"message": "Successfully sent email."}, status=status.HTTP_200_OK
            )
        return Response(
            {"message": "Failed to send OTP email."},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                if not otp.has_expired:
                    ...
                    return Response(
                        {"detail": "Successfully verified your account!"},
                        status=status.HTTP_200_OK,
                    )
                else:
                    return Response(
                ...
                return Response(
                    {"detail": "OTP doesn't exist!"}, status=status.HTTP_404_NOT_FOUND
                )
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
