...
const RegisterOTPModal = ({ isOpen, onClose, email = null }) => {
  ...
  const sendOTP = async () => {
    if (!email) {
      show_toastify('Email is required to send OTP!', 'error');
      return;
    }

    try {
      setLoading(true);
      const response = await axiosInstance.get(`send-otp-registered-user/?email=${encodeURIComponent(email)}`);

      if (response.status === 200) {
        show_toastify(response.data.message, 'success');
        setEmailSent(true);
      } else {
        show_toastify('Unexpected response from the server!', 'error');
        setEmailSent(false);
      }
    } catch (error) {
      if (error.response) {
  ...
  const handleOTPSubmit = async (otp) => {
    try {
      const response = await axiosInstance.put('verify-registered-user-otp/', otp)
      if (response.status === 200) {
      ...
      if (error.response) {
        if (error.response.status === 404) {
          ...
          setVerificationError("OTP has expired. Please resend new OTP to verify!");
        }
      } else {
          ...
          <div className="relative bg-white rounded-lg shadow dark:bg-neutral-700">
            ...
            <div className="p-4 md:p-5 text-center">
              <div className="flex justify-center">
                {verificationError && (
                  <MdOutlineError className='text-4xl text-red-500 text-center' />
                )}
                {!verificationError && (
                  <>
