...
const UpdatePost = () => {
  ...
  const form = useForm()
  const { register, handleSubmit, formState, getValues, reset, watch, setValue } = form
  ...
  const onsubmit = async (formData) => {
    ...
    try {
      ...
      const body = {
        ...
        uploaded_images: fileBlobs 
      };

      const response = await axiosInstance.put(`user-blog/${slug}/`, body, {
        headers: {
      ...
      if (error.response) {
        if (error.response.status === 401) {
          ...
          Swal.fire({
            title: "Error!",
            text: "Sorry, your blog couldn't be updated!",
            icon: "error"
          });
        }
      } else {
  ...
  const fetchUserBlogDetails = async () => {
    try {
      setLoading(true)
      ...
      if (response.status === 200) {
        ...
        setValue('category', response.data.category.slug)
        setValue('uploaded_images', response.data.images)
  ...
  useEffect(() => {
    fetchUserBlogDetails()
  }, [slug])


  if (isPageNotFound) {
    if (user.slug !== blogDetails?.author.slug) {
      return (
    ...
    <>
      ...
      <div className='h-auto flex py-6 mt-5 justify-center  '>
        <div className='text-3xl bg-gray-50 dark:bg-neutral-800 p-8 lg:px-14 max-w-2xl w-full shadow-xl h-auto rounded-2xl' style={{ fontFamily: "Nunito Sans" }}>
          <div>
            <h1 className='  text-[24px] font-semibold text-gray-700 dark:text-gray-300 px-2'>Update Post</h1>
          </div>
          <div className=''>
            <form onSubmit={handleSubmit(onsubmit)}>
              <div className='mt-3'>
                <select className='p-2 px-4 rounded-xl text-[16px] font-semibold shadow bg-gray-100 dark:bg-neutral-700 text-gray-700 dark:text-gray-300 focus:outline-none focus:shadow-outline focus:outline-gray-300 dark:focus:outline-neutral-700 '
                  ...
                  <option disabled value="" className=''>Select a Category</option>
                  {blogCategory && blogCategory.map((category) => (
                    <option key={category.slug} className=' font-medium mb-2' id={category.slug} value={category.slug} selected={getValues('category') === category.slug}>{category.category}</option>
                  ))}
                </select>
              ...
              <div className="mb-4 mt-5 flex justify-center ">
                <button type='submit' className="w-40  hover:bg-neutral-900 hover:text-gray-300 text-gray-900 border-2 border-gray-600 dark:border-gray-300 font-semibold text-sm  rounded-2xl  dark:text-gray-300 dark:hover:bg-gray-300 hover:dark:text-gray-900 transition-colors duration-700 px-3 py-[5px] flex items-center justify-center">
                  <IoCreateSharp className="inline text-[25px] mx-1" /> Update Blog
                </button>
              </div>
            </form>
          </div>
